...
@endphp
@if(!empty($cards))
<section id="why" class="sec-mist re-why">
  <div class="wrap">
    ...
    <div class="svc-carousel page-svc-carousel re-why-carousel" data-sp-carousel data-per-desktop="3">
      <button type="button" class="svc-nav svc-prev" aria-label="Previous">{!! $arrow !!}</button>
      <div class="svc-viewport">
        <div class="svc-track">
          @foreach($cards as $card)
            <article class="why-card svc-slide">
              <div class="w-ico">
                @include('services.partials.digital-marketing.icon', ['key' => $card['icon_key'] ?? 'check'])
              </div>
              @if(!empty($card['num']))
